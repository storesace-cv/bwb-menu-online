-- Fundo configurável por secção: cor e CSS opcional (como rodapé).
-- Colunas em menu_sections; public_menu_initial_by_hostname passa a devolver background_color e background_css nas secções.

ALTER TABLE public.menu_sections
  ADD COLUMN IF NOT EXISTS background_color text,
  ADD COLUMN IF NOT EXISTS background_css text;

-- Recriar public_menu_initial_by_hostname para incluir background_color e background_css no payload das secções.
-- Cópia de 051 com duas linhas adicionais no jsonb_build_object das secções.
    ...
    'sections', (
      SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
          ...
          'background_color', ms.background_color,
          'background_css', ms.background_css
        )
