-- Carga inicial do menu: destaques + secção 1 apenas; lazy load de outras secções.
-- public_menu_initial_by_hostname(host): store, sections (todas), featured_items (todos is_featured), categories (só secção 1).
-- public_menu_section_categories_by_hostname(host, section_id): categorias + items de uma secção (lazy).
...
-- Lazy: categorias (com items) de uma secção por host e section_id
