-- Layout responsivo e espaçamento por zona: coluna layout_definition_mobile; RPC devolve _mobile para cliente escolher por viewport.
-- zoneSpacing e rowSpacingOverrides ficam dentro do jsonb layout_definition (sem alteração de schema).

ALTER TABLE public.menu_presentation_templates
  ADD COLUMN IF NOT EXISTS layout_definition_mobile jsonb DEFAULT NULL;

ALTER TABLE public.menu_featured_presentation_templates
  ADD COLUMN IF NOT EXISTS layout_definition_mobile jsonb DEFAULT NULL;

-- public_menu_by_hostname: incluir presentation_layout_definition_mobile em secções e categorias; featured_layout_definition_mobile no top-level.
...
BEGIN
    ...
    'sections', (
      SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
          ...
          'presentation_layout_definition_mobile', pt.layout_definition_mobile
        )
    ...
    'categories', (
      SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
          ...
          'presentation_layout_definition_mobile', COALESCE(
            (SELECT pt.layout_definition_mobile FROM menu_presentation_templates pt WHERE pt.id = mc.presentation_template_id),
            (SELECT pt.layout_definition_mobile FROM menu_sections ms JOIN menu_presentation_templates pt ON pt.id = ms.presentation_template_id WHERE ms.id = mc.section_id)
          ),
          'items', (
            SELECT COALESCE(jsonb_agg(
              jsonb_build_object(
  ...
  -- Append featured_layout_definition and featured_layout_definition_mobile
  ...
  v_result := v_result || jsonb_build_object(
    ...
    'featured_layout_definition_mobile',
    (SELECT fpt.layout_definition_mobile FROM menu_featured_presentation_templates fpt WHERE fpt.component_key = v_featured_key LIMIT 1)
  );
