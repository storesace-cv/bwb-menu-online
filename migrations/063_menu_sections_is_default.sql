-- Secção apresentada por defeito no menu: is_default em menu_sections.
-- A secção com is_default = true é a primeira no menu e a que carrega categorias iniciais.
-- Se nenhuma tiver is_default, mantém-se o fallback por menor sort_order.

ALTER TABLE menu_sections
  ADD COLUMN IF NOT EXISTS is_default boolean NOT NULL DEFAULT false;

-- Garantir que só uma secção por loja seja default (por aplicação ao definir; opcional: unique partial index)
-- CREATE UNIQUE INDEX IF NOT EXISTS idx_menu_sections_one_default_per_store
--   ON menu_sections (store_id) WHERE is_default = true;

-- public_menu_initial_by_hostname: primeira secção = por is_default, depois por sort_order
...
BEGIN
  ...
  -- Primeira secção: is_default primeiro, depois menor sort_order
