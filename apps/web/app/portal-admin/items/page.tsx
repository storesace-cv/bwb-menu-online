...
export default async function ItemsPage() {
  ...
  const { data: items } = await supabase
    .from("menu_items")
    .select("id, menu_name, menu_description, menu_price, is_visible, is_featured, sort_order")
    .eq("store_id", storeId)
    .order("sort_order");
  ...
  return (
    <div>
      <h1>Itens do menu</h1>
      <p><Link href="/portal-admin/menu">← Menu (categorias)</Link></p>

      <section style={{ marginTop: "1.5rem" }}>
        <h2>Novo item</h2>
        <CreateItemForm storeId={storeId} />
      </section>
